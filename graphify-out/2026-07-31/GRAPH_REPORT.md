...
## Corpus Check
- 223 files · ~119,182 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1945 nodes · 2628 edges · 172 communities (139 shown, 33 thin omitted)
...
Cohesion: 0.10
Nodes (21): axios, file-saver, dependencies, axios, file-saver, html5-qrcode, jspdf-autotable, @opentelemetry/exporter-trace-otlp-http (+13 more)
...
Cohesion: 0.04
Nodes (44): 4. Acceder a los servicios, 5. Crear tu primer usuario, API Gateway (Puerto 8080), Arquitectura, Auth Service (Puerto 8081), Avanzadas, Backend, Base de Datos (+36 more)
...
### Community 147 - "StatementService"
Cohesion: 0.43
...
## Knowledge Gaps
- **763 isolated node(s):** `swarm-init.sh script`, `com.company:spring-boot-app`, `start-local.sh script`, `api-gateway:api-gateway`, `auth-service:auth-service` (+758 more)
...
- **Why does `UserRepository` connect `User` to `.findByEmail`, `AuthService`, `StatementService`?**
  _High betweenness centrality (0.013) - this node is a cross-community bridge._
- **Why does `NotificationRepository` connect `NotificationService` to `.findByEmail`?**
  _High betweenness centrality (0.013) - this node is a cross-community bridge._
- **Why does `UserProfileRepository` connect `UserProfileDto` to `.findByEmail`?**
