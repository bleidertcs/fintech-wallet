# Fintech Wallet - Worker Service (NestJS)

Servicio asíncrono en **NestJS** diseñado bajo **Arquitectura Hexagonal (Ports & Adapters)** para el procesamiento de tareas pesadas en segundo plano: generación de extractos bancarios en formato PDF, registro de auditoría transaccional, y manejo de Dead Letter Queue (DLQ) mediante Apache Kafka.
...
│   │   │   └── rest/              # REST Controllers (Health & Worker Endpoints)
...
2. **Event-Driven & Kafka DLQ**: Consume el tópico `transfer_completed`. En caso de falla en el procesamiento, redirige el mensaje a `transfer-events-dlq` para reintentos y auditoría.
...
nerdctl --namespace k8s.io build -t fintech/worker-service:nestjs .
